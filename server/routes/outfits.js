...
router.get('/', authenticate, (req, res) => {
  try {
    const outfits = db.all('SELECT * FROM saved_outfits WHERE user_id = ? ORDER BY saved_at DESC', [req.user.id]);
    res.json(outfits);
  } catch (err) {
...
router.post('/', authenticate, (req, res) => {
  try {
    ...
    const existing = db.get('SELECT id FROM saved_outfits WHERE user_id = ? AND outfit_id = ?', [req.user.id, outfitId]);
...
router.delete('/:outfitId', authenticate, (req, res) => {
  try {
    const result = db.run('DELETE FROM saved_outfits WHERE user_id = ? AND outfit_id = ?', [req.user.id, req.params.outfitId]);
